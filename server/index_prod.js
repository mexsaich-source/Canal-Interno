...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        // Contar cuántas pantallas ha creado este usuario
        const screensRes = await pool.query('SELECT COUNT(*) FROM screens WHERE created_by = $1', [user.id]);
        const createdCount = parseInt(screensRes.rows[0].count);
        ...
        res.json({ 
            ...
            created_screens_count: createdCount
        });
    } catch (error) {
        console.error(error);
...
app.get('/api/categories', async (req, res) => {
    ...
    try {
        // Enriquecemos la consulta con datos de media para el Dashboard
        ...
        defaultNames.forEach(name => {
            if (!allScreens.some(s => s.category === name)) {
                allScreens.push({ category: name, video_url: '', media_type: 'video', playlist: [], created_by: null });
            }
        });
        ...
        if (user.role === 'editor') {
            const allowed = user.allowed_screens || [];
            allScreens = allScreens.filter(s => allowed.includes(s.category));
        }
...
// NUEVO: Eliminar categoría (SOLO ADMIN)
app.delete('/api/category/:category', authenticateToken, isAdmin, async (req, res) => {
    const { category } = req.params;
